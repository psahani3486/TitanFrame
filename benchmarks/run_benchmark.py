...
def run_pandas_benchmark():
    ...
    res = df[df['l_discount'] > 0.05].groupby('l_returnflag').agg(sum_qty=('l_quantity', 'sum'), sum_price=('l_extendedprice', 'sum')).sort_index()
    end = time.time()
    print(res)
    return end - start
...
def run_titanframe_benchmark():
    ...
    res = lf.filter(tf.col('l_discount') > 0.05).group_by('l_returnflag').agg(tf.col('l_quantity').sum().alias('sum_qty'), tf.col('l_extendedprice').sum().alias('sum_price')).sort('l_returnflag').collect()
    end = time.time()
    print(res)
    return end - start
