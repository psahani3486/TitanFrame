...
class LazyFrame:
    ...
    def rename(self, mapping: dict[str, str]) -> LazyFrame:
        exprs = [col(name).alias(mapping.get(name, name)) for name in self.columns]
        new_plan = Projection(self._plan, exprs)
        return LazyFrame(new_plan)
    ...
    def sort(self, by: str | list[str] | Expr | list[Expr], descending: bool | list[bool]=False) -> LazyFrame:
    ...
    def join(self, other: LazyFrame, on: str | list[str], how: str='inner', suffix: str='_right') -> LazyFrame:
    ...
    def _execute_aggregation(self, plan: Aggregation, child: Table) -> Table:
        ...
        key_names = [k.column_name if isinstance(k, ColumnExpr) else repr(k) for k in plan.group_keys]
    ...
    def __repr__(self) -> str:
        return f'LazyFrame(schema={self.schema})\nPlan:\n{self._plan.display(indent=1)}'
