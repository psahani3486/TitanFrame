    ...
    def execute(self, context: ExecutionContext) -> Iterator[Chunk]:
        ...
        for chunk in self.left.execute(context):
            if context.memory_manager:
                left_buffers.append(context.memory_manager.register(chunk.data))
            else:
                left_buffers.append(chunk.data)
        if not left_buffers:
            return
        right_buffers = []
        for chunk in self.right.execute(context):
            if context.memory_manager:
                right_buffers.append(context.memory_manager.register(chunk.data))
            else:
                right_buffers.append(chunk.data)
        if context.memory_manager:
            left_table = pa.Table.from_batches([b.get_data() for b in left_buffers])
            for b in left_buffers:
        ...
        if context.memory_manager:
            ...
            for b in right_buffers:
                context.memory_manager.free(b)
        else:
            right_table = pa.Table.from_batches(right_buffers)
        pa_join_type = self.how
        if pa_join_type == 'outer':
            pa_join_type = 'full outer'
        res_table = left_table.join(right_table, keys=self.keys, join_type=pa_join_type, right_suffix=self.suffix)
