            ...
            if isinstance(child, Scan) and child.projection is None:
                ...
                for e in plan.exprs:
                    if hasattr(e, 'column_name'):
                        ...
                        return plan
                new_scan = Scan(source=child.source, format=child.format, schema=child._schema, projection=cols, predicate=child.predicate, limit=child.limit, chunk_size=child.chunk_size)
