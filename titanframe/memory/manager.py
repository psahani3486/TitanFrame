...
class MemoryManager:
    ...
    def spill_if_needed(self, required_bytes: int):
        if self.current_usage + required_bytes <= self.budget_bytes:
            return
        spillable = [b for b in self.buffers if b.tier == Tier.RAM and b.pinned_count == 0]
        ...
        if self.current_usage + required_bytes > self.budget_bytes:
            raise MemoryError(f'Memory budget exceeded! Current: {self.current_usage}, Required: {required_bytes}, Budget: {self.budget_bytes}. Spillable buffers might all be pinned.')
