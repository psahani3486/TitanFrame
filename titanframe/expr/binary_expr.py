...
from titanframe.expr.base import BinaryExpr, Expr, Op, _wrap
__all__ = ['BinaryExpr', 'Op', 'add', 'sub', 'mul', 'true_div', 'floor_div', 'mod', 'pow_', 'eq', 'ne', 'lt', 'le', 'gt', 'ge', 'and_', 'or_', 'xor', 'infer_binary_dtype', 'is_arithmetic_op', 'is_comparison_op', 'is_logical_op']
_ARITHMETIC_OPS = frozenset({Op.ADD, Op.SUB, Op.MUL, Op.TRUE_DIV, Op.FLOOR_DIV, Op.MOD, Op.POW})
_COMPARISON_OPS = frozenset({Op.EQ, Op.NE, Op.LT, Op.LE, Op.GT, Op.GE})
_LOGICAL_OPS = frozenset({Op.AND, Op.OR, Op.XOR})
