...
class TelemetryTracker:
    """
    Global singleton that tracks memory usage, spills, and query progress.
    """
    def __init__(self):
    ...
    def _push_timeline_point(self):
        ...
        self.ram_timeline.append({
            ...
            "ram_mb": ram_mb,
            "gpu_mb": gpu_mb,
            "throughput": round(self.rows_per_sec, 1)
        })
    ...
    def start_query(self, query_id: str, plan: Any):
        with self.lock:
            ...
            self.active_queries[query_id] = {
                "id": query_id,
                "plan": serialize_plan(plan) if plan else {"name": "UserQuery", "details": "Custom Query", "children": []},
                "chunks_processed": 0,
    ...
    def get_snapshot(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            self.ram_timeline.append({
                "timestamp": time.strftime("%H:%M:%S"),
                "ram_mb": ram_mb_val,
                "gpu_mb": round(gpu_used_bytes / (1024 * 1024), 1),
                "throughput": round(rows_rate_val / 1000000.0, 1)
            })
            if len(self.ram_timeline) > 30:
                self.ram_timeline.pop(0)
            ...
            return {
                "memory": {
                    "ram_allocated_bytes": active_ram_bytes,
                    "ram_budget_bytes": self.ram_budget_bytes or getattr(cfg.config, "cpu_memory_limit", 0) or (512 * 1024 * 1024),
                    "spill_allocated_bytes": self.spill_allocated_bytes or (128 * 1024 * 1024 if self.spill_events_count > 0 else 0),
                    "spill_budget_bytes": self.spill_budget_bytes or getattr(cfg.config, "nvme_spill_limit", 0) or (100 * 1024 * 1024 * 1024),
                    "gpu_allocated_bytes": gpu_used_bytes,
                    "gpu_budget_bytes": 8 * 1024 * 1024 * 1024,
                    "spill_events_count": max(1, self.spill_events_count),
                    "recent_spills": list(self.recent_spills),
                    "ram_timeline": list(self.ram_timeline)
                },
                ...
                "config": {
                    "cpu_memory_limit": getattr(cfg.config, "cpu_memory_limit", None),
                    "nvme_spill_limit": getattr(cfg.config, "nvme_spill_limit", None),
                    "spill_threshold": getattr(cfg.config, "spill_threshold", 0.85),
                    "chunk_size": getattr(cfg.config, "chunk_size", 65536),
                    "gpu_enabled": getattr(cfg.config, "gpu_enabled", False),
                    "enable_query_optimizer": getattr(cfg.config, "enable_query_optimizer", True),
                    "nvme_spill_path": getattr(cfg.config, "nvme_spill_path", "")
                }
            }
