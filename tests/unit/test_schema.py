"""
Tests for TitanFrame Schema (schema.py)
========================================

Tests covering:
    - Construction from dicts, tuples, Arrow schemas
    - Property access (names, dtypes, num_columns)
    - Lookups (by name, by index)
    - Algebra (select, drop, rename, merge, intersect, diff, append)
    - Validation (compatibility, subset checking)
    - Arrow round-trip
    - Inference from Python dicts
"""
...
class TestSchemaAlgebra:
    ...
    def test_drop_nonexistent_is_ok(self, left: Schema):
        """Dropping a non-existent column is a no-op."""
