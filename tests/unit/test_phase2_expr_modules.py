import pytest
from titanframe.core.dtypes import Int8, Int16, Int32, Int64, UInt8, UInt16, UInt32, UInt64, Float32, Float64, Bool, Utf8, Date, Datetime, Null
...
class TestBinaryExprModule:
    ...
    def test_import(self):
        from titanframe.expr.binary_expr import add, sub, mul, true_div, floor_div, mod, pow_, eq, ne, lt, le, gt, ge, and_, or_, xor, infer_binary_dtype, is_arithmetic_op, is_comparison_op, is_logical_op
...
class TestUnaryExprModule:
    ...
    def test_import(self):
        from titanframe.expr.unary_expr import neg, not_, abs_, ceil, floor, sqrt, log, exp, is_null, is_not_null, infer_unary_dtype, is_null_check_op, is_math_op
...
class TestAggExprModule:
    ...
    def test_import(self):
        from titanframe.expr.agg_expr import sum_, mean, min_, max_, count, count_distinct, first, last, std, var, median, quantile, any_, all_, QuantileExpr, PartialAggResult, infer_agg_dtype, partial_agg_state_fields, is_reducible_op
...
class TestCastExprModule:
    ...
    def test_import(self):
        from titanframe.expr.cast_expr import CastExpr, TryCastExpr, cast, try_cast, validate_cast, is_identity_cast, CastError, NUMERIC_CAST_PAIRS, TEMPORAL_CAST_PAIRS, STRING_CAST_PAIRS, to_int64, to_float64, to_string, to_bool, to_date, to_datetime
...
class TestExprModuleExports:
    ...
    def test_base_types_exported(self):
        from titanframe.expr import Expr, BinaryExpr, UnaryExpr, AggExpr, CastExpr, AliasExpr, SortExpr, Op, UnaryOp, AggOp, SortOrder
    ...
    def test_binary_factories_exported(self):
        from titanframe.expr import add, sub, mul, true_div, floor_div, mod, pow_, eq, ne, lt, le, gt, ge, and_, or_, xor

    def test_unary_factories_exported(self):
        from titanframe.expr import neg, not_, abs_, ceil, floor, sqrt, log, exp, is_null, is_not_null

    def test_agg_factories_exported(self):
        from titanframe.expr import sum_, mean, min_, max_, count, count_distinct, first, last, std, var, median, quantile, any_, all_, QuantileExpr

    def test_cast_factories_exported(self):
        from titanframe.expr import cast, try_cast, TryCastExpr, validate_cast, to_int64, to_float64, to_string, to_bool

    def test_type_inference_exported(self):
        from titanframe.expr import infer_binary_dtype, infer_unary_dtype, infer_agg_dtype
...
class TestEvaluatorIntegration:
    ...
    def test_lazy_quantile(self):
        ...
        from titanframe.expr.agg_expr import quantile
        df = tf.DataFrame({'x': [10, 20, 30, 40, 50]})
        result = df.select(quantile(col('x'), q=0.25).alias('q25'), quantile(col('x'), q=0.75).alias('q75'))
